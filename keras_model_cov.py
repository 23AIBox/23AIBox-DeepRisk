...
def get_model(snp_num, gene_num, cov_num, mask, gene_ids):

    ipt_geno = layers.Input(shape=(snp_num,))
    ipt_covs = layers.Input(shape=(cov_num,))
    ...
    x_covs = layers.Dense(units=gene_num)(ipt_covs)
    x_covs = layers.Reshape(target_shape=((gene_num, 1)))(x_covs)

    x = layers.Add()([x, x_covs])
    ...
    x = layers.Concatenate(axis=-1)([x, ipt_covs])
    ...
    model = keras.Model(inputs=[ipt_geno, ipt_covs], outputs=[opt])
...
def train_fold(model: keras.Model, class_weight, x_train, y_train, x_val, y_val, x_test, y_test):
    model.fit(
        ...
        callbacks=[
            callbacks.ModelCheckpoint(
                filepath='./checkpoint/weights_cov.h5',
    ...
    model.load_weights(filepath='./checkpoint/weights_cov.h5')
...
if __name__ == "__main__":
    ...
    cov = np.concatenate([PCs.values[:, :4], covariances[['sex', 'platform', 'age', 'degree', 'townsend']].values], axis=1)

    genotype, phenotype, beta, PCs, covariances = dataset.extract(snps, return_PCs=True, return_covariates=True)
    x, y = genotype.values.T, phenotype.values
    print(len(y), y.sum(), len(cov))
    ...
    for fold, (train_index, test_index) in enumerate(kfold.split(x, y)):
        ...
        x_train, x_test = [x[train_index], cov[train_index]], [x[test_index], cov[test_index]]

        class_weight = compute_class_weight(y_train)

        model = get_model(snp_num, gene_num, cov.shape[1], mask, gene_ids.values())
        # if fold == 0:
        #     model.summary()
    ...
    result['pred'] = preds
    result.to_csv('our_result/%sprobs_cov.csv' % (suffix))
